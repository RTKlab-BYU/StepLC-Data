...
INPUT_FILES = {"10 min":"10min_January/report.tsv","12 min":"ultimate/report.tsv"}
...
def plot_RTstdev_boxplot(RTs, plot_options, username):
    ...
    if plot_options["median label"] == "True" or \
        ...
        total_labels = [{"x": x, "y": total*1.5, "text": str(
...
for eachType in INPUT_FILES.keys():
    ...
    eachFilter = FILTERS[i]
    currentData = allData[allData["Run"].str.contains(eachFilter)]
    currentData = currentData[~currentData["Run"].str.contains("5856")]
    currentData = currentData[~currentData["Run"].str.contains("5857")]
    
    if currentData.size >0:
        RT_Length = max(currentData["RT"].tolist()) - min(currentData["RT"].tolist())
        currentData["Width"] =  currentData["RT.Stop"]-currentData["RT.Start"]
        ...
        currentData = currentData.groupby("Precursor.Id").agg(
                                                                # Median= ("RT","median"),
                                                                # Mean=("RT","mean"),
                                                                Stdev=("Width",np.median)).reset_index()
        currentData["Stdev"] = currentData["Stdev"]*60
    ...
    if currentData.size >0:
        ...
        if i == 0:
            allPeaks = currentData.drop_duplicates().dropna()
        else:
        
            allPeaks = pd.merge(allPeaks,currentData.drop_duplicates().dropna(),how="inner")
...
plot_options={    
        ...
        "Y Title": "Peak Width",
        "color": ["#AC4314","dark grey","#3E6990","blue", "cyan", "black", "gray", "purple", "pink",
